...
export default async function ExperienceListing(data: ExperienceData, index) {
    ...
    var internal = (
        <FadeInContent delayOffset={index * 0.08}>
                ...
                <div className="flex flex-row items-center space-x-2">
                    <div className="text-lg font-semibold text-teal-500 dark:text-teal-400 align-top">{data.title}</div>
                    {
                        data.link == null ? <div /> : <div className="align-text-bottom">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-2 h-2 group-hover:w-4 group-hover:h-4 group-hover:transform group-hover:duration-250 transition-all">
